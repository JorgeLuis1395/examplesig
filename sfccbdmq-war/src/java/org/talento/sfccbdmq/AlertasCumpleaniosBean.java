 ...
 * @author edwin
 */
@ManagedBean(name = "alertasCumpleaniosSfccbdmq")
@ViewScoped
public class AlertasCumpleaniosBean {

    /**
     * Creates a new instance of AuxiliarCalendarioBean
     */
    public AlertasCumpleaniosBean() {
    }
    private AuxiliarCalendario calendario;
    private List<AuxiliarCalendario> calendarioMes;
    private List<Auxiliar> calendarioLista;
    private Formulario formulario = new Formulario();
    private Perfiles perfil;
    @EJB
    private EmpleadosFacade ejbEmpledos;
    ...
    public String buscar() {
        ...
        for (int i = 0; i < ultimo_dia; i++) {
            ...
            try {
                lista = ejbEmpledos.traerCumpleanieros(mes, i);
            } catch (ConsultarException ex) {
                Logger.getLogger(AlertasCumpleaniosBean.class.getName()).log(Level.SEVERE, null, ex);
            }
            boolean entro = false;
            for (Entidades g : lista) {
//                garantias += " " + g.toString() + " ";
                garantias += "<p>" + g.toString() + "</p>";
                entro = true;
//                garantias += "<p><Strong>Nombres :</Strong>" + g.toString() + "</p>";
            }
            if (entro) {
                Auxiliar a = new Auxiliar();
                a.setIndice(diaMostrar);
                a.setTitulo1(garantias);
                calendarioLista.add(a);
            }
            guarda = false;
    ...
    private void activar() {
        ...
        String nombreForma = "AlertasCumpleaniosVista";
        if (perfil == null) {
...
//        }
    ...
    /**
     * @return the calendarioLista
     */
    public List<Auxiliar> getCalendarioLista() {
        return calendarioLista;
    }

    /**
     * @param calendarioLista the calendarioLista to set
     */
    public void setCalendarioLista(List<Auxiliar> calendarioLista) {
