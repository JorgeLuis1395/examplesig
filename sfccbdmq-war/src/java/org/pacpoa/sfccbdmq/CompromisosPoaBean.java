 ...
 * @author luis
 */
@ManagedBean(name = "compromisosPoa")
@ViewScoped
public class CompromisosPoaBean {
    ...
    private List<Ejecucionpoa> carga(int i, int pageSize, SortCriteria[] scs, Map<String, String> map) {
        ...
        String where = "o.fecha between :desde and :hasta and o.ejecutado=0.00";
        ...
        try {
            ...
            Logger.getLogger(CompromisosPoaBean.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private boolean validar() {
        ...
        if (ejecucion.getComprometido() == null || ejecucion.getComprometido().doubleValue() <= 0.00) {
            MensajesErrores.advertencia("Valor debe ser mayor a 0");
            return true;
        }
        double certificado = calculosPoa.traerCertificaciones(asignacion.getProyecto().getCodigo(), asignacion.getPartida().getCodigo(), configuracionBean.getConfiguracion().getPinicialpresupuesto(), configuracionBean.getConfiguracion().getPfinalpresupuesto(), asignacion.getFuente());
        double comprometido;
        if (formulario.isNuevo()) {
            comprometido = calculosPoa.traerCompromisos(asignacion.getProyecto().getCodigo(), asignacion.getPartida().getCodigo(), configuracionBean.getConfiguracion().getPinicialpresupuesto(), configuracionBean.getConfiguracion().getPfinalpresupuesto(), asignacion.getFuente());

        } else {
            comprometido = calculosPoa.traerCompromisos(asignacion.getProyecto().getCodigo(), asignacion.getPartida().getCodigo(), configuracionBean.getConfiguracion().getPinicialpresupuesto(), configuracionBean.getConfiguracion().getPfinalpresupuesto(), asignacion.getFuente(), ejecucion.getId());
        }

        if ((comprometido + ejecucion.getComprometido().doubleValue()) > certificado) {
            MensajesErrores.advertencia("Valor comprometido sobrepasa el valor certificado");
            return true;
        }
