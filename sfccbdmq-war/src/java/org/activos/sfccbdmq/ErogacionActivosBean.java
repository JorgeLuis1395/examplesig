 ...
 * @author edwin
 */
@ManagedBean(name = "erogacionActivosSfccbdmq")
@ViewScoped
public class ErogacionActivosBean {

    /**
     * Creates a new instance of ActivoBean
     */
    public ErogacionActivosBean() {
...
//    private int tamano;
    ...
    private void activar() {
        ...
        String nombreForma = "ErogacionActivosVista";
        if (perfil == null) {
...
//    }
    ...
    public String modifica() {
        ...
        try {
            ...
            getTacking().setDescripcion("COLOCA EROGACIÓN");
            getTacking().setFecha(new Date());
            getTacking().setTipo(9);
...
//        }
        if (erogacion==null){
            MensajesErrores.advertencia("Seleccione una erogación");
            return true;
        }
        return false;
    }
    ...
    public String grabar() {
        ...
        try {
            erogacion.setValoractivo(activo.getValoralta());
            erogacion.setVidautilactivo(activo.getVidautil());
            erogacion.setActivo(activo);
            ejbErogaciones.edit(erogacion, getSeguridadbean().getLogueado().getUserid());
            activo.setValoralta(new BigDecimal(activo.getValoralta().doubleValue()+erogacion.getValor().doubleValue()));
            activo.setVidautil(activo.getVidautil()+erogacion.getVidautil());
            ejbActivos.edit(activo, getSeguridadbean().getLogueado().getUserid());
            setTacking(new Trackingactivos());
            getTacking().setActivo(activo);
            getTacking().setDescripcion("COLOCA EROGACIÓN");
            getTacking().setCuenta1(null);
            getTacking().setCuenta2(erogacion.getDescripcion());
            getTacking().setValornuevo(erogacion.getValor().floatValue());
            getTacking().setFecha(new Date());
            getTacking().setTipo(9);
            getTacking().setUsuario(seguridadbean.getLogueado().getUserid());
...
//        buscar();
    ...
    public String quitar(Erogaciones erogacion) {
        

        try {
            setTacking(new Trackingactivos());
            tacking.setCuenta1(erogacion.getDescripcion());
            tacking.setValor(erogacion.getValor().floatValue());
            activo.setValoralta(new BigDecimal(erogacion.getValoractivo().doubleValue()));
            activo.setVidautil(erogacion.getVidautilactivo());
            erogacion.setValoractivo(null);
            erogacion.setVidautilactivo(null);
            erogacion.setActivo(null);
            ejbErogaciones.edit(erogacion, getSeguridadbean().getLogueado().getUserid());
            ejbActivos.edit(activo, getSeguridadbean().getLogueado().getUserid());
            
            getTacking().setActivo(activo);
            getTacking().setDescripcion("REVERSA EROGACIÓN");
            tacking.setCuenta2(null);
            getTacking().setFecha(new Date());
            getTacking().setTipo(-9);
            getTacking().setUsuario(seguridadbean.getLogueado().getUserid());
