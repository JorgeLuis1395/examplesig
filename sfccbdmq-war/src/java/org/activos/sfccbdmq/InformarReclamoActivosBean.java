 ...
 * @author edwin
 */
@ManagedBean(name = "informarReclamoActivosSfccbdmq")
@ViewScoped
public class InformarReclamoActivosBean {

    /**
     * Creates a new instance of ActivoBean
     */
    public InformarReclamoActivosBean() {
...
//    private String marca;
    ...
    private Polizas polizaSelecciona;
    ...
    private void activar() {
        ...
        String nombreForma = "InformarReclamoActivosVista";
        if (perfil == null) {
...
//                }
                ...
                activos.setRowCount(total);
                try {
                    List<Activopoliza> liataAp = ejbActivoPoliza.encontarParametros(parametros);
                    List<Activos> activos = new LinkedList<>();
                    for (Activopoliza ap : liataAp) {
                        activos.add(ap.getActivo());
                    }
                    return activos;
                } catch (ConsultarException ex) {
                    MensajesErrores.fatal(ex.getMessage() + " : " + ex.getCause());
...
//            tacking.setDescripcion(custodio);
    ...
    private boolean validar() {
        if ((tacking.getDescripcion() == null)) {
            MensajesErrores.advertencia("Necesario observaciones");
            return true;
        }
        if ((polizaSelecciona == null)) {
            MensajesErrores.advertencia("Necesario poliza");
            return true;
        }
        if ((tacking.getCuenta2() == null)) {
            MensajesErrores.advertencia("Necesario Denunciante");
            return true;
        }
    ...
    public String grabar() {
        ...
        try {
            Codigos textoCodigo = getCodigosBean().traerCodigo("MAIL", "RECLAMO");
            if (textoCodigo == null) {
                MensajesErrores.fatal("No configurado texto para email en codigos");
                return null;
            }
            SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
            String texto = textoCodigo.getParametros().replace("#informante#", tacking.getCuenta2());
            texto = texto.replace("#serie#", activo.getNumeroserie() == null ? "SIN NUMERO SERIE" : activo.getNumeroserie());
            texto = texto.replace("#descripcion#", activo.getDescripcion() == null ? "SIN DESCRIPCION" : activo.getDescripcion());
            texto = texto.replace("#fechainclusion#", sdf.format(polizaSelecciona.getDesde()));
            texto = texto.replace("#denunciante#", tacking.getUsuario());
            texto = texto.replace("#observaciones#", tacking.getDescripcion());
            texto = texto.replace("#solicita#", tacking.getDescripcion());
            getTacking().setDescripcion("Informa Reclamo a : " + polizaSelecciona.getDescripcion() + " Informado por: " + tacking.getCuenta2());
            ejbTracking.create(getTacking(), getSeguridadbean().getLogueado().getUserid());
            // correo
            ejbCorreos.enviarCorreo(textoCodigo.getNombre(),
            ...
            MensajesErrores.fatal(ex.getMessage());
            Logger.getLogger(InformarReclamoActivosBean.class.getName()).log(Level.SEVERE, null, ex);
        }
        formulario.cancelar();
//        buscar();
        return null;
...
//    }
    ...
    public SelectItem[] getComboPolizas() {
        if (activo == null) {
            return null;
        }
        Map parametros = new HashMap();
        ...
        List<Activopoliza> lpz;
        try {
            lpz = ejbActivoPoliza.encontarParametros(parametros);
            List<Polizas> lp = new LinkedList<>();
            for (Activopoliza a : lpz) {
                lp.add(a.getPoliza());
            }
    ...
    /**
     * @return the polizaSelecciona
     */
    public Polizas getPolizaSelecciona() {
        return polizaSelecciona;
    }

    /**
     * @param polizaSelecciona the polizaSelecciona to set
     */
    public void setPolizaSelecciona(Polizas polizaSelecciona) {
        this.polizaSelecciona = polizaSelecciona;
    }
