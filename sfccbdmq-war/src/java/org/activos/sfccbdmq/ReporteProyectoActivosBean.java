 ...
 * @author edwin
 */
@ManagedBean(name = "reporteProyectoActivosSfccbdmq")
@ViewScoped
public class ReporteProyectoActivosBean {

    /**
     * Creates a new instance of ActivoBean
     */
    public ReporteProyectoActivosBean() {
    ...
    private void activar() {
        ...
        String nombreForma = "ReporteProyectoActivosVista";
        if (perfil == null) {
...
//        }
        try {
            ...
            String where = "o.baja is null and o.fechaalta is not null and o.ccosto is not null";
            if (proyectoBean.getProyectoSeleccionado() != null) {
                where += " and o.ccosto like :ccosto";
                parametros.put("ccosto", proyectoBean.getProyectoSeleccionado().getCodigo()+"%");
            }
            ...
            DocumentoPDF pdf = new DocumentoPDF("ACTIVOS POR CENTRO DE COSTO ", seguridadbean.getLogueado().getUserid());
            ...
            for (Activos a : listaActivos) {
                if (!a.getCcosto().equals(ccosto)) {
                    if (ccosto != null) {
                        ...
                        columnas.add(new AuxiliarReporte("String", 1, AuxiliarReporte.ALIGN_RIGHT, 6, false, ""));
                        columnas.add(new AuxiliarReporte("String", 4, AuxiliarReporte.ALIGN_RIGHT, 6, true, "Total " + ccosto));
                        columnas.add(new AuxiliarReporte("Double", 1, AuxiliarReporte.ALIGN_RIGHT, 6, true, totalProveedor));
                    }
                    Proyectos p=proyectoBean.traerCodigoSinAnio(a.getCcosto());
                    
                    columnas.add(new AuxiliarReporte("String", 1, AuxiliarReporte.ALIGN_LEFT, 6, false, ""));
                    columnas.add(new AuxiliarReporte("String", 4, AuxiliarReporte.ALIGN_LEFT, 8, true, a.getCcosto()));
                    columnas.add(new AuxiliarReporte("String", 4, AuxiliarReporte.ALIGN_LEFT, 8, true, p==null?"":p.getNombre()));
                    columnas.add(new AuxiliarReporte("String", 1, AuxiliarReporte.ALIGN_LEFT, 6, false, ""));
                    columnas.add(new AuxiliarReporte("String", 1, AuxiliarReporte.ALIGN_RIGHT, 6, false, ""));
                    columnas.add(new AuxiliarReporte("String", 1, AuxiliarReporte.ALIGN_RIGHT, 6, false, ""));
                    columnas.add(new AuxiliarReporte("String", 1, AuxiliarReporte.ALIGN_RIGHT, 6, false, ""));
                    ...
                    columnas.add(new AuxiliarReporte("String", 1, AuxiliarReporte.ALIGN_RIGHT, 6, false, ""));
                    columnas.add(new AuxiliarReporte("String", 1, AuxiliarReporte.ALIGN_RIGHT, 6, false, ""));
//                    pdf.agregaParrafo("Centro de Costo : " + a.getProveedor().getEmpresa().toString() + "\n\n");
                    totalProveedor = 0;
                    ccosto = a.getCcosto();
            ...
            pdf.agregarTablaReporte(titulos, columnas, 10, 100, "REPORTE DE ACTIVOS POR CENTRO DE COSTO");
            reporte = pdf.traerRecurso();
            formulario.insertar();
        } catch (ConsultarException | IOException | DocumentException ex) {
            MensajesErrores.fatal(ex.getMessage());
            Logger.getLogger(ReporteProyectoActivosBean.class.getName()).log(Level.SEVERE, null, ex);
        }
        return null;
    }
