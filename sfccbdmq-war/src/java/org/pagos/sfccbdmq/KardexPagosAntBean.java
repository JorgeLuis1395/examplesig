 ...
 * @author edwin
 */
@ManagedBean(name = "kardexPagosAntSfccbdmq")
@ViewScoped
public class KardexPagosAntBean {

    /**
     * Creates a new instance of CertificacionesBean
     */
    public KardexPagosAntBean() {
...
//                    if (pro == null) {
//                        ////////////////////////////Beneficiario 
//                        Entidades entidad = empleadosBean.traerCedulaEntidad(p.getDocumento());
//                        if (entidad != null) {
...
//                            p.setBancotransferencia(codigoBanco);
//                            p.setCuentatrans(numeroCuenta);
//                            p.setTcuentatrans(Integer.parseInt(tipoCuenta));
//                        }
//                        //////////////////////////////// Sigue en propuestas
//                    } else {
...
//                    }
            ...
            Logger.getLogger(KardexPagosAntBean.class.getName()).log(Level.SEVERE, null, ex);
        }
                    ...
                    if (k.isSeleccionar()) {
                        ...
                        if (kardexPropuesta.getTipomov().getSpi()) {
                            if (k.getOrigen().equals("PAGO ROLES")) {
                                k.setEgreso(Integer.valueOf(df.format(spi.getNumero()) + df.format(++i)));
                            } else {
                                k.setEgreso(Integer.valueOf(String.valueOf(anio) + df.format(spi.getNumero()) + df.format(++i)));
                            }
                            k.setSpi(spi);
                        }
...
//                    k.setDocumento(df.format(spi.getId()) + "" + df.format(++i));
//                        kardex = k;
                        insertarKardex();
                    }
                    // disponible el pagos
...
//            asiento.setModulo(perfil.getMenu().getIdpadre().getModulo());
            ...
            Cuentas cuentaBanco = getCuentasBean().traerCodigo(kardexPropuesta.getBanco().getCuenta());
...
//                            // proveedor
//                            String xuNombre = getProveedoresBean().taerRuc(r.getAuxiliar()).getNombre();
//                            r.setAuxiliarNombre(xuNombre);
//                        } else if (c.getAuxiliares().getParametros().equals("P")) {
//                            // empleado
//                            String xuNombre = getEmpleadosBean().traerCedula(r.getAuxiliar());
//                            r.setAuxiliarNombre(xuNombre);
//                        } else if (c.getAuxiliares().getParametros().equals("P")) {
//                            // cliente
//                            String xuNombre = getProveedoresBean().taerRuc(r.getAuxiliar()).getNombre();
...
//                        }
                ...
                parametros.put(";where", "o.cabecera.idmodulo=:id "
                        ...
                        + " and o.cabecera.opcion like 'PAGO PROVEEDORES RECLASIFICACION%'");
                parametros.put("id", kardex.getId());
//                parametros.put("modulo", perfil.getMenu().getIdpadre().getModulo());
...
//        getKardex().setFechamov(new Date());
    ...
    public String modificaKardex(Kardexbanco kardex) {
...
//        getKardex().setUsuariograba(seguridadbean.getLogueado());
    ...
    public String modificaKardexSeleccion(Kardexbanco kardex) {
        this.kardex = kardex;
        pagos = new LinkedList<>();
        pagosb = new LinkedList<>();
//        formulario.insertar();
...
//                                p.getObligacion().getConcepto()));
    ...
    public String imprimirKardex1(Kardexbanco kardex) {
//        pagos = new LinkedList<>();
        pagosb = new LinkedList<>();
//        formulario.editar();
        setKardex(kardex);
...
//            for (Pagosvencimientos p : pagos) {
//                htmlString += "<tr>";
//                htmlString += "<td>" + p.getObligacion().getConcepto() + "</td>";
//                htmlString += "<td>" + p.getObligacion().getProveedor().getEmpresa().toString() + "</td>";
//                htmlString += "<td>" + p.getObligacion().getDocumento() + "</td>";
//                htmlString += "<td>" + (p.getObligacion().getContrato() == null ? "" : p.getObligacion().getContrato().toString()) + "</td>";
//                htmlString += "<td>" + df.format(p.getValor()) + "</td>";
//                htmlString += "</tr>";
//            }
            htmlString += "</table>";
            if (kardex.getProveedor() != null) {
...
//            parametros.put("documento", kardex.getEgreso() == null ? kardex.getId().toString() : kardex.getEgreso().toString());
...
//        buscarPromesas();
    ...
    public String insertarKardexSeleccion() {
...
//            renglones=new LinkedList<>();
            ...
            for (Pagosvencimientos p : pagos) {
                entro = true;
                double valorPago = p.getValor().doubleValue();
...
//                totalPagos += p.getValor().doubleValue() - (vanticipo + vmulta);
                totalPagos += p.getValor().doubleValue();
                parametros = new HashMap();
                ...
                for (Retencionescompras rc : rl) {
                    ...
                    if (rc.getRetencion() != null) {
                        if (mesActual == mesObligacion) {
                            ...
                            Cuentas cuentaContraria = cuentaRet; // pongo la cuenta de la retención es temporall getCuentasBean().traerCodigo(ctaContraria);

                            // armar la cuenta
                            Renglones rasRetencion = new Renglones();
                            rasRetencion.setCuenta(ctaRetencion);
                            double valorRetencion = rc.getValor().doubleValue();
                            rasRetencion.setValor(new BigDecimal(valorRetencion));
                            rasRetencion.setReferencia(kardex.getObservaciones());
//                                rasRetencion.setDetallecompromiso(rc.getDetallecompromiso());
//                                rasRetencion.setCentrocosto(rc.getCc());
                            rasRetencion.setDetallecompromiso(null);
                            rasRetencion.setFecha(kardex.getFechamov());
                            if (cuentaRet.getAuxiliares() != null) {
                                rasRetencion.setAuxiliar(p.getObligacion().getProveedor() == null ? (p.getObligacion().getCompromiso().getBeneficiario().getEntidad().getPin()) : (p.getObligacion().getProveedor().getEmpresa().getRuc()));
                            }
                            ...
                            rasContraria.setReferencia(kardex.getObservaciones());
//                                rasContraria.setCentrocosto(rc.getCc());
                            rasRetencion.setDetallecompromiso(null);
//                            rasRetencion.setDetallecompromiso(rc.getDetallecompromiso());
                            rasContraria.setFecha(kardex.getFechamov());
                            if (cuentaContraria.getAuxiliares() != null) {
                                rasContraria.setAuxiliar(p.getObligacion().getProveedor() == null ? (p.getObligacion().getCompromiso().getBeneficiario().getEntidad().getPin()) : (p.getObligacion().getProveedor().getEmpresa().getRuc()));
                            }
                            estaEnRenglonesReclasificacion(rasContraria);
                            if (rc.getRetencionimpuesto() != null) {
                                // armada la cuenta de retención
                                ctaRetencion = f.getCxpinicio() + presupuesto + rc.getRetencionimpuesto().getCuenta() + rc.getRetencionimpuesto().getCodigo();
                                ctaContraria = codigoReclasificacion.getParametros() + rc.getRetencionimpuesto().getReclasificacion() + rc.getRetencionimpuesto().getCodigo();
                                cuentaRet = getCuentasBean().traerCodigo(ctaRetencion);
                                cuentaContraria = cuentaRet;//Temporal ver el efecto getCuentasBean().traerCodigo(ctaContraria);
                                // es retencion
                                // armar la cuenta
                                rasRetencion = new Renglones();
                                rasRetencion.setCuenta(ctaRetencion);
                                valorRetencion = rc.getValoriva().doubleValue();
                                rasRetencion.setValor(new BigDecimal(valorRetencion));
                                rasRetencion.setReferencia(kardex.getObservaciones());
//                                    rasRetencion.setCentrocosto(rc.getCc());
//                                rasRetencion.setDetallecompromiso(rc.getDetallecompromiso());
...
//                    pago.setKardexbanco(getKardex());
                double valorAnticipo = p.getValoranticipo() == null ? 0 : p.getValoranticipo().doubleValue();
//***************************************QUITADO PARA CONTABILIZAR EN PAGO LOTES
...
//                ejbRenglones.create(r1, seguridadbean.getLogueado().getUserid());
                ...
                for (Retencionescompras rc : rl) {
                    ...
                    r.setFecha(asiento.getFecha());
                    r.setReferencia(kardex.getObservaciones());
                    if (rc.getBaseimponible() == null) {
                        rc.setBaseimponible(BigDecimal.ZERO);
                    }
                    if (rc.getValoriva() == null) {
                        rc.setValoriva(BigDecimal.ZERO);
                    }
                    if (rc.getBaseimponible0() == null) {
                        rc.setBaseimponible0(BigDecimal.ZERO);
                    }
                    if (rc.getIva() == null) {
                        rc.setIva(BigDecimal.ZERO);
                    }
                    if (rc.getValor() == null) {
                        rc.setValor(BigDecimal.ZERO);
                    }
                    ...
                    if (cuentaP.getAuxiliares() != null) {
                        r.setAuxiliar(p.getObligacion().getProveedor() == null ? (p.getObligacion().getCompromiso().getBeneficiario().getEntidad().getPin()) : (p.getObligacion().getProveedor().getEmpresa().getRuc()));
                    }
                    estaEnRas(r);
                    totalObligacion += valor;

                }
                //Se estan generando mas renglones revisar 
                if (!retenciones) {
                    // tarer los documentos electronicpos
                    parametros = new HashMap();
                    parametros.put(";where", "o.obligacion=:obligacion");
                    parametros.put("obligacion", p.getObligacion());
...
//                        + " de la Cuenta " + (getKardex().getBanco().getCorriente() ? " Corriente No: " : " Ahorros No: ")
//                        + getKardex().getBanco().getNumerocuenta());
                        r1.setValor(new BigDecimal((p.getValor().doubleValue() - valorAnticipo)));
                        r1.setCuenta(ctaxPagar.getCodigo());
                        r1.setPresupuesto(ctaxPagar.getPresupuesto());
                        if (ctaxPagar.getAuxiliares() != null) {
//                            r1.setAuxiliar(p.getObligacion().getProveedor() == null ? p.getKardexbanco().getDocumento()
...
//                getRenglones().add(r1);
            ...
            if (kardex.getAnticipo() != null) {
                // llenar renglones de anticipos y bancos
...
//                ejbRenglones.create(r1, seguridadbean.getLogueado().getUserid());
                renglones.add(r);
                renglones.add(r1);
            } else if (kardex.getOrigen().contains("PAGO ROLES")) {
...
//            kardex.setValor(new BigDecimal(totalObligacion));
//            kardex.setValor(new BigDecimal(totalPagos));
